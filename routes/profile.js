...
export default (pool) => {
  // Получить профиль пользователя по id (заглушка без авторизации)
  ...
  // Обновить профиль (только имя и email для примера)
  ...
  router.put('/:userId/password', async (req, res) => {
    ...
    try {
      ...
      if (!match) return res.status(403).json({ error: 'Старый пароль неверен' });
